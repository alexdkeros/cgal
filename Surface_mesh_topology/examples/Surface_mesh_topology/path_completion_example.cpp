...
void create_path_1(Path_on_surface<LCC_3_cmap>& p)
{
  ...
  p.extend_negative_turn(1);  // Extend the path
...
int main()
{
 ...
 std::cout<<"is p1 (pink) closed? "<<p1.is_closed()<<" simple? "<<p1.is_simple()<<std::endl;
 std::cout<<"is p2 (green) closed? "<<p2.is_closed()<<" simple? "<<p2.is_simple()<<std::endl;
 std::cout<<"is p3 (yellow) closed? "<<p3.is_closed()<<" simple? "<<p3.is_simple()<<std::endl;
